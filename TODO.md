...
- Company register: the card and the bot offer the official company search (CAC, URSB, ORC, BRS) with the name copied, free. Optional keys add the row itself: OpenCorporates (index, labelled with its date) or Mono (live CAC, billed).
...
- [x] Telegram: with `TELEGRAM_BOT_TOKEN` and `TELEGRAM_WEBHOOK_SECRET` in Vercel and the site redeployed, run `curl -X POST https://daju-bice.vercel.app/api/telegram/setup -H "Authorization: Bearer <TELEGRAM_WEBHOOK_SECRET>"` once. Then open the bot and press /start.
...
- [ ] Company register row, optional and free: apply for an OpenCorporates API key (opencorporates.com, public-benefit programme) and put it in Vercel as `OPENCORPORATES_API_TOKEN`; the card then shows the CAC row labelled as an index with its date. Without it the card still offers the official CAC search with the name copied. Mono stays off.
